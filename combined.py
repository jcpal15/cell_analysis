...
def extract_identifier(filename):
    """
    Extract identifier from the filename according to these rules:
    - If there's an underscore, extract everything after the last underscore (without extension)
    - If there's no underscore, use the whole filename without extension
    
    Examples:
    - "baseline-try6-epi_F1.ims" -> "F1"
    - "baseline_atps.ims" -> "atps"
    """
    ...
    # If there's an underscore, take the last part
    if len(parts) > 1:
        return parts[-1]
    else:
        # If no underscore, return the whole stem
        return stem
...
def analyze_multichannel_depletion_h5(h5_path, ref_channel=1, meas_channel=0, 
    ...
    # Extract identifier for labeling
    ...
    try:
        ...
        if bleach_correction:
            ...
            plt.title(f'Reference Channel Bleaching Correction - {identifier}', fontsize=16)
        ...
        plt.title(f'Mean Channel Intensities Over Time - {identifier}')
        ...
        plt.title(f'{ratio_title} - {identifier}')
        plt.xlabel('Time Point')
        ...
        return {
            ...
            'cells_detected': len(valid_labels) if 'valid_labels' in locals() else 0,
...
def process_and_combine_ims_files(folder_path, ref_channel=1, meas_channel=0, 
                               resolution_level=0, z_slice=0, 
                               save_cell_images=True, bleach_correction=True,
                               bleach_model='exponential', poly_order=2,
                               normalization_point=0):
    """
    Process each IMS file in a folder individually and then create combined graphs
    """
    # Find all IMS files
    ...
    # Create combined graphs and summary if we have results
    if all_results:
        # Create timestamp for the summary and combined graphs
        current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Extract folder name for naming combined graphs
        folder_name = os.path.basename(os.path.normpath(folder_path))
        
        # Create combined output directory
        combined_dir = f'combined_analysis_{folder_name}_{current_time}'
        if not os.path.exists(combined_dir):
            os.makedirs(combined_dir)
        
        # Create the combined graphs
        create_combined_graphs(all_results, combined_dir, folder_name, bleach_correction)
        ...
        for result in all_results:
            summary_data.append({
                ...
                'Time Points': result['num_time_points'],
        ...
        # Save the summary
        summary_file = os.path.join(combined_dir, f"{folder_name}_analysis_summary_{current_time}.csv")
        ...
        print(f"\nCombined graphs saved to {combined_dir}")
    else:
        print("\nNo successful analyses to combine.")
    
    print("\nAll processing complete!")

def create_combined_graphs(results, output_dir, folder_name, bleach_corrected=True):
    """
    Create combined graphs showing data from all files together
    """
    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # 1. Create combined ratio plot
    ...
    # Save the combined ratio plot
    label = 'corrected' if bleach_corrected else 'orig'
    ratio_file = os.path.join(output_dir, f'{folder_name}_combined_ratio_plot_{label}_{current_time}.png')
    plt.savefig(ratio_file, dpi=300)
    plt.close()
    
    print(f"Combined ratio plot saved as {ratio_file}")
    
    # 2. Create combined reference channel plot
    print("\nCreating combined reference channel plot...")
    plt.figure(figsize=(12, 8))
    
    # Plot each file's reference intensity
    for i, result in enumerate(results):
        color = colors[i % len(colors)]
        identifier = result['identifier']
        
        if bleach_corrected and result.get('mean_ref_corr') is not None:
            # Use corrected reference values if available
            mean_ref = result['mean_ref_corr']
            std_ref = result['std_ref_corr']
        else:
            # Otherwise use original reference values
    ...
    # Add plot details
    title = f'Comparison of Reference Channel Intensities - {folder_name}'
    if bleach_corrected:
        title += " (Bleach Corrected)"
    plt.title(title, fontsize=16)
    plt.xlabel('Time Point', fontsize=14)
    plt.ylabel('Mean Reference Intensity', fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend(title='Sample ID', loc='best', fontsize=12)
    ...
    # Save the combined reference plot
    ref_file = os.path.join(output_dir, f'{folder_name}_combined_reference_plot_{label}_{current_time}.png')
    plt.savefig(ref_file, dpi=300)
    plt.close()
    
    print(f"Combined reference channel plot saved as {ref_file}")
    
    # 3. Create combined measurement channel plot
    print("\nCreating combined measurement channel plot...")
    plt.figure(figsize=(12, 8))
    
    # Plot each file's measurement intensity
    for i, result in enumerate(results):
        color = colors[i % len(colors)]
        identifier = result['identifier']
        mean_meas = result['mean_meas']
        std_meas = result['std_meas']
    ...
    # Add plot details
    plt.title(f'Comparison of Measurement Channel Intensities - {folder_name}', fontsize=16)
    plt.xlabel('Time Point', fontsize=14)
    plt.ylabel('Mean Measurement Intensity', fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend(title='Sample ID', loc='best', fontsize=12)
    ...
    # Save the combined measurement plot
    meas_file = os.path.join(output_dir, f'{folder_name}_combined_measurement_plot_{current_time}.png')
    plt.savefig(meas_file, dpi=300)
    plt.close()
    
    print(f"Combined measurement channel plot saved as {meas_file}")
    
    # 4. Save all combined data to CSV for further analysis
    combined_data = []
    
    # Collect all data points for each file
    for result in results:
        identifier = result['identifier']
        
        for time_point in result['time_points']:
            if time_point in result['mean_ratios'].index:
                data_point = {
                    'identifier': identifier,
                    'time_point': time_point,
                    'mean_ratio': result['mean_ratios'][time_point],
                    'std_ratio': result['std_ratios'][time_point],
                    'mean_reference': result['mean_ref'][time_point],
                    'std_reference': result['std_ref'][time_point],
                    'mean_measurement': result['mean_meas'][time_point],
                    'std_measurement': result['std_meas'][time_point],
                }
                
                # Add corrected reference data if available
                if bleach_corrected and result.get('mean_ref_corr') is not None:
                    data_point['mean_reference_corrected'] = result['mean_ref_corr'][time_point]
                    data_point['std_reference_corrected'] = result['std_ref_corr'][time_point]
                
                combined_data.append(data_point)
    
    # Convert to DataFrame and save
    combined_df = pd.DataFrame(combined_data)
    combined_csv = os.path.join(output_dir, f'{folder_name}_combined_data_{current_time}.csv')
    combined_df.to_csv(combined_csv, index=False)
    
    print(f"Combined data saved as {combined_csv}")

# Usage example:
if __name__ == "__main__":
    # Process all IMS files and create combined graphs
    ...
    # Process all files in the folder and create combined graphs
    process_and_combine_ims_files(
        folder_path, 
        ref_channel=ref_channel,
        meas_channel=meas_channel,
        save_cell_images=save_cell_images,
